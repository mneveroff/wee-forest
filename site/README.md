...
Note: the `/lens` proxy applies to `astro dev` only, not `astro preview` or the static `dist/` build. Area API calls use `/lens/area/calculate_areas` (no trailing slash).
